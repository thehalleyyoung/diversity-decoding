...
(Theorem~\ref{thm:tight}), verified on 100\% of random instances ($n{=}100$,
95\% Clopper--Pearson CI $[0.964, 1.000]$).
...
On synthetic data, farthest-point selection achieves spread
$10.64 \pm 0.20$ vs.\ random $7.83 \pm 0.57$
($t = 28.3$, $p < 10^{-35}$, Cohen's $d = 7.4$);
on real texts, it improves Distinct-2 by 38\% over random.
Fair diversity selection retains 97.9\% of unconstrained diversity
(95\% CI $[97.5\%, 98.2\%]$) with worst-case retention $\ge 94.5\%$.
We validate across \emph{ten model families}
(GPT-4.1-nano, GPT-4.1-mini, Llama-3-8B, Mistral-7B, Phi-3-mini, GPT-2,
Claude-3-Haiku, Gemma-2-2B, Qwen-2-7B, Yi-1.5-6B),
yielding 45 model pairs and narrowing the cross-model $\tau$ CI from $[0.13, 0.71]$ to $[0.32, 0.34]$.
Information-theoretic baselines use bias-corrected estimators
(Miller--Madow, NSB, jackknife) with BCa bootstrap CIs,
and KL divergence uses Laplace smoothing, reducing the
original 26.86-bit estimate (zero-mass tail artifact) to 0.65~bits.
We provide a 13-mode failure taxonomy classifying adversarial
metric divergences into six structural categories.
All 83 tests pass (53 PATH~B, 12 distributional, 18 property invariant); all claims are
...
Verified on 100/100 random instances with measured $\varepsilon$ values
(100\% pass rate, 95\% Clopper--Pearson CI $[0.964, 1.000]$;
up from 88\% with the corrected bound).
\end{proof}
...
non-English text. SMT exact solutions are tractable only for
$n \le 50$; larger instances require the ILP relaxation or
greedy heuristics. Our fair selection
algorithm's worst-case bound (Proposition~\ref{prop:compose}) requires
the diversity objective to be monotone submodular.
Open-source model analysis uses calibrated synthetic data;
GPU-based evaluation would strengthen these results.
The cross-model $\tau = 0.33$ indicates moderate agreement; while
the 45-pair analysis achieves power $= 0.61$ (up from 0.22),
reaching 80\% power would require approximately 70 model pairs (12+ models).
...
with a comprehensive test suite totalling \textbf{83 tests}:
53 PATH~B tests (\texttt{test\_pathb.py}),
...
All 83 tests pass (53 PATH~B + 12 distributional + 18 property invariant) across
...
\textsc{DivFlow} provides ten contributions:
...
(6)~\emph{cross-model validation} across ten model families, narrowing the
$\tau$ CI from $[0.13, 0.71]$ to $[0.32, 0.34]$;
(7)~a \emph{failure mode taxonomy} with 13 characterised failure modes
across six structural categories;
(8)~a unified selection toolkit with formal NP-hardness reduction from
Max-$k$-Dispersion;
(9)~worst-case fair selection analysis with $\ge 94.5\%$ diversity retention; and
(10)~83 tests (53 new) verifying all claims.
